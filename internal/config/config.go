...
func Parse(args []string) (Config, error) {
	...
	set.BoolVar(&cfg.DisableLogging, "disable-logging", cfg.DisableLogging, "disable request logging")
...
func applyEnvironment(cfg *Config) {
	...
	if value := os.Getenv("LOCKD_DISABLE_LOGGING"); value != "" {
		if parsed, err := strconv.ParseBool(value); err == nil {
			cfg.DisableLogging = parsed
		}
	}
}
